      ...
      tap(
        ...
        error => {
          if (error instanceof HttpErrorResponse) {
            ...
            if(error.status === 403){
              this.alertService.error("Acesso não autorizado - Permissão negada");
            }
            if(error.status === 500){
              this.alertService.error("Não foi possivel realizar a requisição - Tente novamente mais tarde");
            }
          }
        })
    );
